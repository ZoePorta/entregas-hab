...
app.get("/search", (req, res) => {
  ...
  const pageContent = searchResults(results);
  res
    .status(200)
...
//------
//Esta página muestra la ficha del pokemon
app.get("/pokemon/:id", (req, res) => {
  const id = req.params.id;

  const pokemon = pokedex.filter((pokemon) => pokemon.id === +id)[0];

  const pageContent = fichaPokemon(pokemon);

  res.send(pageLayout(`Ficha de ${pokemon.name.english}`, pageContent));
});
